...
TipBox.prototype.renderUI = function(tipType){   
    ...
    if(this.config.hasBtn){
        ...
        $('button.okoButton').on('click',function(){window.location.href = "/";});
    }
    //定时消失  
    _this = this;  
    !this.config.setTime && typeof this.config.callBack === "function" && (this.config.setTime = 1);
    this.config.setTime && setTimeout( function(){ window.location.href = "/" }, _this.config.setTime );
};  
...
TipBox.prototype.bindUI = function(){  
    ...
    //点击空白立即取消  
    this.config.clickDomCancel && this._mask && this._mask.click(function(){window.location.href = "/"});
};  
